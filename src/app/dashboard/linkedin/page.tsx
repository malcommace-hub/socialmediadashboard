...
export default function LinkedInPage() {
  ...
  async function saveMonthly() {
    ...
    // Invalidate cached history/overview so charts, KPIs and the month score
  ...
  function isFeaturedId(post: LinkedInPost): string | null {
  ...
  async function handleSaveFeature(post: LinkedInPost) {
    ...
    const { data } = await addFeaturedContent({
      ...
      description: post.title || null,
      views: post.impressions || null,
      er_pct: post.er_decimal > 0 ? +(post.er_decimal * 100).toFixed(2) : null,
                ...
                <tbody>
                  ...
                  {sorted.map((post, idx) => {
                    ...
                    {featuredFormPostId === post.id && (
                      <tr>
                        <td colSpan={10} className="px-3 pb-3 pt-1.5 bg-amber-50 border-b border-amber-100">
                          <div className="flex items-center gap-2">
